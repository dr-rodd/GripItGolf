...
| `lib/intro.ts` / `app/components/SiteIntro.tsx` | The site intro — the travelling dot, run once per device on the trip hub: the wordmark's emerald dot swells out of the logo into a big wordless circle that criss-crosses the screen while a stationary block of writing walks the newcomer round, one paragraph per tap; the tab being described stays sharp in a feathered hole through a soft blur veil, warmed by an emerald glow, and the circle shrinks home at the end. Gated by the `gg_intro` cookie (hub page checks the jar on the server, skip and finish both write it, so neither re-invites). **Everything is measured at runtime, never hardcoded**: tab icons by `data-intro-tab` (TabBar.tsx), the logo dot as the `<g fill="#0a9d56">` inside `.gd-mark` — and a target that cannot be found costs that stop its spotlight, never the user their screen. Motion is the putt curve, 480–550ms travels and a 2% settle — a documented exception scoped to the component (Motion, `docs/design-system.md`), which is why its durations live there and not in `globals.css`, where `test:branding`'s ceilings would refuse them. The veil and glow live in `globals.css` (`.intro-*`); the spotlight's feather is an SVG alpha mask in the component, SVG rather than a CSS gradient so the no-gradients grep keeps its teeth |
