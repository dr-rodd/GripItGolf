...
| `lib/quota.ts` | Quota play, the only copy: bogey 1, par 2, birdie 4, eagle 6 off the **gross**, chasing 36 minus course handicap, result signed against that target. **The handicap enters once, in the target** — an allowance reduces it there, never the per-hole points, and the stored `scores.points` are Stableford's so a quota board never reads them. Individual boards only — `docs/leaderboards.md` |
