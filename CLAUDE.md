...
| `lib/intro.ts` / `app/components/SiteIntro.tsx` | The site intro — a newcomer's guided lap of the app, run once per device on the trip hub: green dots with cream writing, a brown arrow pointing at the tab each one describes. Gated by the `gg_intro` cookie, checked **on the server** by the hub page so the intro is in the first paint; skip and finish both write the cookie, so neither re-invites. The arrows mirror the tab bar's geometry (five equal columns, `max-w-lg`, centres at the odd tenths) rather than measuring it — change the bar's layout and the intro must move with it. Colours are fixed literals in `globals.css` (`.intro-*`), the same in both themes, deliberately |
