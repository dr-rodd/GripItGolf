...
- **A tournament's hub is the Event Hub** — the same `/trip/[code]` route reading `trips.kind`: a **Notices** section fed by `event_messages` (leads the stack, opens first when it has any), **Schedule** instead of "Your Itinerary", and Travel & Accommodation only when a stay or journey is actually planned. **The organiser PIN is the settings passcode** — required at tournament creation (a trip still chooses), one code behind both Trip Setup and `/trip/[code]/organiser`, the same soft lock as ever (`lib/passcode.ts`'s note holds). The organiser area posts and removes notices and picks each round's start: **shotgun** writes its one time to the round's itinerary item, where the countdown, weather and schedule already read it; **tee sheet** stores the choice (`rounds.start_format`) and says so on the schedule — the sheet itself, groups and times, is deliberately still to come. The organiser area also holds the **Format** card (`/trip/[code]/organiser/bracket`) — for a match play event, the bracket setup form: seven answers saved whole to `trips.bracket_setup` (jsonb, migration 047, applied by hand; reads fail soft in their own query, never inside a page's main select): **strict** (organiser pre-pairs and pre-courses every round) or **relaxed** (players self-organise and link their own cards) mode, a 16/32/64/128 field ceiling, organiser-entered or self-join entry, an optional qualifying event referenced by its own code whose standings seed the draw (randomised among the qualifiers, or fully seeded — first plays last), a deadline per bracket round, and finalise-now-or-leave-open — finalised is read-only and for keeps, open means the field keeps growing until the organiser comes back. `lib/bracketSetup.ts` is the only copy of those rules; the draw itself — generating matches from the setup — still belongs to `lib/matchplay.ts` and is work to come. **For a league event the same screen is a summary, never a form** — a save there would overwrite the league with a bracket, so the page branches on `parseLeagueSetup` before it ever renders the match play form.
- **A league is the other tournament format, created whole** — the tournament door asks league-or-match-play first (`CreateFlow`, the same client-side URL read, route still static), and League opens its own four-step wizard (`CreateLeagueForm`): **not the trip wizard re-worn** — event details with single-day or multi-day (single is deliberately lightweight: one venue, one date, one leaderboard), a venue per day with a same-venue-every-day toggle, the field (organiser-entered or self-join with an optional require-approval toggle — the approval answer is stored and said on screen; the join-flow gate is built on top of it later, the same posture as `tee_sheet`), and for multi-day one more question: how the days relate on the leaderboard — **separate days, one running total, or days-and-overall** (`DayBoards`). What it reuses from the trip wizard is the proven underneath: `CourseSelect`, `HandicapField`, the roster rules, `toItemRow`, the insertion order. Creation writes the trip (`kind: 'tournament'`, the league setup whole in `bracket_setup`, the starter individual-Stableford-total board in `leaderboards` — a league plays from the moment it exists), the players, a golf item and round per day, and the handicap snapshots. `lib/leagueSetup.ts` is the only copy of the rules, sharing `trips.bracket_setup` with the knockout discriminated on `format` — each parser refuses the other's object. Scoring formats within league play, and the per-day leaderboard presentation the `dayBoards` answer feeds, are deliberately still to come. **The field never sees Trip Setup on an event**: the tab bar drops the tab (four tabs, decided by the trip layout's cached `kind` read) and the setup screen is reached only through the organiser area — the seed of a full admin portal. **Participant screens carry no modifiers on an event**: scoring offers no add-round, a round page no casual switch, and `/trip/[code]/teams` stands behind the PIN. Pages learn the kind through `app/trip/[tripCode]/kind.ts` — the one cached, fail-soft lookup, asked *alongside* a page's own first query (never named inside its select, which would fail the page on an un-migrated database). `lib/eventHub.ts` is the only copy of the rules themselves.
...
| `lib/intro.ts` / `app/components/SiteIntro.tsx` | The site intro — the travelling dot, run once per device on the trip hub: after a beat the wordmark's emerald dot stirs, swells and blooms into a big bubble of the logo's own emerald carrying cream writing — a title and one paragraph per tap — parked high favouring the top-middle-left and hopping aside only when a page changes or it would cover the feature being ringed (each paragraph highlights its feature on the replica via artboard-coordinate `focus` regions), while **the real page being described fills the screen behind it** — hand-traced SVG replicas of live screenshots (`public/intro/`, seven screens, a few KB each, regenerated by `npm run intro:shots` from `scripts/make-intro-shots.mjs`), full-bleed down to the user's own tab bar, chosen per paragraph (`shots` on each step), cover-fit with a sacrificial bottom so phone shapes crop padding, not content. The page's edges soften behind a faint vignette blur; the tab being described stays sharp in a feathered hole, warmed by an emerald glow; the bubble shrinks home at the end. Gated by the `gg_intro` cookie — **once per device, not per trip** (hub page checks the jar on the server; skip and finish both write it) — and `?intro` on the hub URL overrides it, for demos and for testing on a phone that has already seen it. **Everything is measured at runtime, never hardcoded**: tab icons by `data-intro-tab` (TabBar.tsx), the logo dot as the `<g fill="#0a9d56">` inside `.gd-mark` — and a target that cannot be found costs that stop its spotlight, never the user their screen. Motion is the putt curve, 480–750ms travels behind a deliberate ~600ms opening pause, and a 2% settle — a documented exception scoped to the component (Motion, `docs/design-system.md`), which is why its durations live there and not in `globals.css`, where `test:branding`'s ceilings would refuse them. The veil, bubble and glow colours live in `globals.css` (`.intro-*`); the vignette and its tab hole are an SVG alpha mask in the component, SVG rather than a CSS gradient so the no-gradients grep keeps its teeth |
...
| `lib/leagueSetup.ts` | **A league event's rules, the only copy** — the answers the rounds cannot carry (entry method, require-approval, how a multi-day's days relate on the board), the day ceiling (`MAX_ROUNDS`, the same number not a second copy), and `starterBoards()`, the individual-Stableford-total board every league is created with. Deliberately does **not** store single-vs-multi day, day count or venues — the rounds and the trip dates are the one copy of those. Shares `trips.bracket_setup` with the knockout, discriminated on `format` |
