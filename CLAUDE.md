...
| `lib/handicap.ts` | Shots received on a hole, and how a handicap is written and read. **A plus handicap is negative** and gives shots back from SI 18 down. **A handicap nobody has given yet is `null` — pending, and pending is not scratch** (`isHandicapPending`, migration 051): the creation form always allowed a blank box and stored `?? 0`, so the player the lead player was least sure about became the best handicap on the trip. Blank now stores null, shows as `HANDICAP_PENDING_LABEL` on the hub roster, Trip Setup and the picker, gets **no `round_handicaps` row** (the trigger already returns early without one), and **cannot be put on a scorecard**: `pendingInCard`/`pendingCardReason` grey Start Round and name who is missing one, and `playerSetups` filters them out so nothing downstream can be handed one. Set when they claim their name, or by the organiser. `readHandicapField` is the one copy of **blank ≠ mistyped** — a field that saves as it is left cannot treat a cleared box and a half-typed `1.` alike |
