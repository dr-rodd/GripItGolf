...
| `lib/intro.ts` / `app/components/SiteIntro.tsx` | The site intro — the travelling dot, run once per device on the trip hub: the wordmark's emerald dot swells out of the logo into a big circle of cream writing, tours the tabs with a brown arrow reaching to each real icon, and shrinks home. Gated by the `gg_intro` cookie (hub page checks the jar on the server, skip and finish both write it, so neither re-invites). **Everything is measured at runtime, never hardcoded**: tab icons by `data-intro-tab` (TabBar.tsx), the logo dot as the `<g fill="#0a9d56">` inside `.gd-mark` — and a target that cannot be found costs that stop its arrow, never the user their screen. Motion is the putt curve, 480–550ms travels and a 2% settle — a documented exception scoped to the component (Motion, `docs/design-system.md`), which is why its durations live there and not in `globals.css`, where `test:branding`'s ceilings would refuse them. Colours are fixed literals in `globals.css` (`.intro-*`), the same in both themes, deliberately |
