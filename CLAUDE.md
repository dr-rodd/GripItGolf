...
| `lib/displayNames.ts` | **What the leaderboard prints for a player — the only copy.** A nickname the player chose themselves (preferences gear, `players.nickname`, migration 047), else first name + the start of the last ("Ross O"), grown *together* on ties ("Ross O'G" / "Ross O'B"). The stored name never changes; the scorecard sheet keeps it in full. Cousin of `shortNames` in `lib/matchplayEntrants.ts`, which starts from bare first names on purpose. **Fetch `players.nickname` on its own, fail-soft** — naming it in a shared select fails the page pre-047. Team selection used to rename real players for board space; that door is closed |
