...
function TeamColumn({
  team, players, totalHandicap, onRename, onRecolour,
}: {
  team: Team
  players: Player[]
...
export default function TripTeamsClient({
  ...
  // ── Auto-balance ─────────────────────────────────────────────
  // Snake draft by handicap so team totals land close together.

  async function autoBalance() {
    if (teams.length === 0 || busy) return
    if (!window.confirm('Spread all players across the teams by handicap? This replaces the current assignment.')) return
    setBusy(true)
    ...
    if (results.some(r => r.error)) {
      setPlayers(prev)
      flashError('Could not balance teams — try again')
    }
    setBusy(false)
  }
    ...
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="space-y-6">
        ...
        <p className="text-white/30 text-xs text-center">
          Drag players between teams. On a phone, press and hold briefly first.
        </p>
        ...
        {teams.length === 0 && (
          <p className="text-white/40 text-sm text-center py-8">
            Pick a number of teams above to get started.
          </p>
        )}
      </div>
