...
### …and the second pass, on what the same path downloads

Five things, none of which lost a score — they only made getting to the card slower, which on a bad radio is its own kind of unusable. All five are closed and pinned in `test:score-outbox`:

- **The Scoring tab downloaded every score in the trip.** `scores.round_id` and `live_scores.round_id` across every round — some seventeen hundred rows for a week's golf for sixteen — reduced to a Set of at most half a dozen ids, on the critical path of the tab a group opens most often. It is a `head: true` count per round now: the reply is a header and no body. More requests, each a rounding error, and on a struggling connection small-and-many beats large-and-few every time. `live_scores` is only asked about for a round that is actually open, because the tile rule ignores it otherwise.
- **Three things loaded for a sheet behind a `+`.** The platform course catalogue, the whole itinerary and every player's handicap, on every visit, serialised into the tab's HTML, for a sheet most visits never open. All three live in `AddRound` now and fetch on the tap — `usePlatformCourses` took an `enabled` flag for it. **The itinerary is the `before` half of a diff**, so a fetch that failed or has not finished refuses to save rather than falling back to `[]`, which would read as "delete the trip's whole itinerary".
- **Two sequential queries where PostgREST would join.** Both scoring pages looked a trip up by code and then asked for its rounds; they embed now. One round trip saved on each, and a round trip is the expensive unit here. Note the trap: supabase-js reads the select string as a *literal type*, so `'a, ' + 'b'` leaves it with nothing to infer from and every field on the result fails to typecheck. One string, no concatenation.
- **`round_handicaps` for the whole trip** on the round page, when every consumer filters to the round being scored before using one. Scoped to that round.
- **The resume path made three sequential client round trips.** The handicaps and the card both need the locked players and neither needs the other, so they go together now — and resuming is precisely the moment somebody is standing on a tee with one bar.
