...
**The site intro is the second documented exception, after the landing page's collapse.** The solid sweep (`app/components/SiteIntro.tsx`) is a guided tour, not a control answering a touch: its exit and card slides run 420–500ms on one strong ease-out — `cubic-bezier(0.16, 1, 0.3, 1)`, named the putt curve in the component: quick off the face, a long confident deceleration — and the birth alone runs 1.5s on its own slow-in slow-out, because the putt curve's fast start read as an explosion where a bloom was wanted. All of it is scoped to that component and to nothing else: the durations, the curve and the card-slide keyframes live in the component (its own `<style>` tag) rather than in this stylesheet, so `test:branding`'s ceilings over `globals.css` still hold everything ordinary to 400ms. The writing on the sweep sits still — an idle drift was tried and retired, because text being read must not move; a 2% arrival overshoot was tried and retired with the old parked-bubble design. Under `prefers-reduced-motion` the whole choreography collapses to thoughts appearing in place. Nothing outside the intro may borrow the curve or the durations.
