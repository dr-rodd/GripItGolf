...
**There is one repeat that is not a breath, and it is named rather than left as a gap.** A leaderboard name too long for its column glides out and back on its own (`.name-scroll-track`, driven by `ScrollingName` in `TripLeaderboardClient.tsx`). That is a distance at a reading pace, not a pulse: the duration is computed per name from how far it has to travel, clamped so a two-character overrun does not creep and a very long name does not park itself. A two-second ceiling would whip a long one past unread, so `test:branding` lifts it out of the breath rule and instead holds the stylesheet's fallback inside the component's clamp — the two cannot drift into a rule the other would refuse. **It only ever runs on a name that measured wider than its column**: a board where every name fits is a board where nothing moves, and a leaderboard that fidgets for no reason is worse than one that clips.
