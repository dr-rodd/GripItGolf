...
**A better-ball board names its own count, and its own finish.** `countingScores` on the board — how many of the team's scores make the composite card on each hole, asked in settings directly under the format (chips for 1 and 2, a keypad behind "Something else" up to `MAX_COUNTING_SCORES`). **Absent reads as 2**, which is what the maths always counted for a board that was never asked, so every stored trip scores exactly as it did; the explicit default is likewise kept off the object. `aggregateFinish` is the grandstand finish, asked right after it — closing holes on which **every** score counts rather than the best few, so a trailing team can still catch up; it can only raise a team's total, never lower it. **Absent means off** (`aggregateFinishOf`), 1 to `MAX_AGGREGATE_FINISH` (17) when on, and off is never stored. **A stored 18 reads as off, not as a softened 17**: a finish of the whole round is the retired aggregate format silently overriding the counting answer on every hole, and the only 18s ever stored came from the pre-fix keypad rounding a mistyped figure up — dropping them restored the best-x format those boards actually named. `countingScoresOf`/`aggregateFinishOf` resolve them, `describeBetterBall` is the one copy of the wording, and a count above a team's size simply caps out at everyone. Both questions belong to `better_ball` alone — hero and cut-the-dead-weight judge whole cards.

**A team board can name its rows by the players.** `hideTeamName` on the board, asked in settings of every team league format — presentation only, nothing about scoring. Hidden, the row is `shortNames` of the members (so two Rosses stay two people) and the sub-label is dropped rather than repeating them; absent means the team name shows with the players underneath, as it always did. Player names themselves are editable on the team screen — tap a name on its tile — guarded by the roster's no-two-same-names rule; safe mid-trip because scores and the claim cookie key on player id. The board's name cells scroll a long name into view rather than truncating, and the total column sizes itself to the widest total on the board.

**The opened team scorecard prints the board's holes, not a sum.** `teamHolePoints` in `lib/teamScoring.ts` is the one copy of the team's per-hole figure — best N (finish included), the hero's own card, everyone but the cut — and `teamRoundPoints` is its sum *by construction*, so the card and the round figure on the board cannot disagree. The sheet reads it through `teamCardHolePoints` in `lib/boardRows.ts` (same settings, allowance and basis as `teamRows`). The card used to add every member's points on every hole, which is only what aggregate means — on a best-1 board it read as counting two while the leaderboard counted one.

**Team format options survive the switch.** `teamScoringFor(board, legacy)` hands back the trip's old `team_scoring` verbatim when the format matches (per-board `countingScores` and `aggregateFinish` still win, since only a board that was asked carries them), so a trip mid-way through is never silently re-scored. Options from a *different* format are not carried across. `aggregate` was retired from the form but stays in `ALL_TEAM_FORMATS` so trips running it still read and score as themselves.
...
`lib/tiebreak.ts` is the only copy. Three answers, stored as `tieBreak` — and **asked only of a board that pays by position** (`offersTieBreak`). A tie is a prizes question: on a board that just adds rounds up, level players share the place and no back nine decides anything they are playing for, so `parseLeaderboards` also drops the answer off any totals board — including the countback the form used to seed onto every board it made. The in-play panel inside the scoring card keeps its countback regardless: mid-round, whatever the trip's boards say, countback is what a group on the eighteenth green means by who won.
...
**Absent means Even Split**, which is what every board did before the question existed — so no trip already on the platform is re-scored. A prizes board being *made* is seeded with **Tiebreak** (`DEFAULT_TIE_BREAK`) when "points by position" is picked, and the answer is cleared again if the board stops paying by position. Two different defaults for two different questions, deliberately. An Even Split is never stored, the same way an allowance of 100 is not.
