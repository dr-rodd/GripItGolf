...
// "golf" leaves the frame. Far enough left to be gone before it would clip
// the words that stay, and it fades on the way out.
const golfExit = -(allInk.right - allInk.left) * 0.85

const morph = {
  stackedBox,
  lineBox,
  words: [
    { id: 'green', dx: 0,          dy: 0,                    fades: false, body: wordBody(green) },
    { id: 'dot',   dx: dotShift,   dy: baseY - dot[0].y,     fades: false, body: wordBody(dot) },
    { id: 'golf',  dx: golfExit,   dy: baseY - golf[0].y,    fades: true,  body: wordBody(golf) },
    { id: 'mark',  dx: markShift,  dy: baseY - mark[0].y,    fades: false, body: wordBody(mark) },
  ],
}
...
export type MorphWord = {
  ...
  /** Offset from the stacked position to the line position, in user units. */
  dx: number
  dy: number
...
export const MORPH_WORDS: MorphWord[] = ${
  JSON.stringify(morph.words.map(w => ({ ...w, dx: +w.dx.toFixed(2), dy: +w.dy.toFixed(2) })), null, 2)
}
`)
